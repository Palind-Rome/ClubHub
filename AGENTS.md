...
- **Issue 驱动**：任何实质性改动（新增功能、修复缺陷、文档变更、数据库变更等）在开始实施之前，必须先确认是否存在对应 Issue——没有则创建。无论该改动来自用户指令还是自主发现，此规则均适用。只有阶段性合入主线（dev → main）、CI 自动化提交等不针对特定问题的场景可以跳过。
- 新分支应使用 `类型/编号-简短描述` 格式，例如 `feature/42-activity-checkin`；创建后立即用 `gh pr create --draft` 创建 draft PR。
...
- Agent **禁止**在未经用户确认的情况下创建 Issue、执行 `git commit` 或 `git push`，并且**绝对禁止对 dev 和 main 分支使用 `push -f`**，且**非必要情况下不准使用 `push -f`**。
- **Issue 先行**：Agent 在开始任何实质性改动前，必须先判断是否属于必须关联 Issue 的场景（通常是）。如果是，先创建 Issue 并获得用户确认，再继续后续操作。不允许在未创建 Issue 的情况下直接实施改动并开 PR。
