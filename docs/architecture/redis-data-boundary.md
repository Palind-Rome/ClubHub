...
> 基线任务：Issue #152
> 状态：#153–#156 已完成基础组件、部署、通用缓存、会话、权限、预览、限流和幂等接入
...
| 学习预览             | Redis 保存 Token 摘要绑定的 OSS 引用和元数据，绝对 30 分钟                         | Token、用户、学习项三者必须匹配；全局最多 4096 条          | Development 本地文件路径仍保持单实例语义                                       |
