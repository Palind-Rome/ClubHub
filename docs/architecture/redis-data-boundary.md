...
> 基线任务：Issue #152
> 状态：#153–#157 已完成基础组件、部署、通用缓存、会话、权限、预览、限流、幂等和分布式锁接入
...
以下内容基于 #153–#157 完成后的当前实现。

| 范围                 | 当前实现                                                                           | 现有保护                                                  | 已知边界                                                                       |
| -------------------- | ---------------------------------------------------------------------------------- | --------------------------------------------------------- | ------------------------------------------------------------------------------ |
| 登录                 | HMAC Token 带随机会话 ID；Redis allowlist 30 分钟滑动、12 小时绝对上限             | 注销、停用、强制下线即时撤销；每用户最多 10 个会话        | 开关启用和回滚均要求重新登录                                                    |
| 权限                 | 账号状态、系统角色、社团角色和展开权限缓存 5 分钟                                  | 损坏或未命中回源 Oracle；缩权写入前后双失效               | 安全失效失败时返回 503 并回滚                                                   |
...
| 场地预约             | 按场地和北京时间日期加锁，在 Serializable Oracle 事务内复查冲突                    | 多日固定顺序取锁；审批同时锁预约 ID；提交前确认租约        | 时间重叠查询索引仍需另开数据库 Issue 评估                                       |
| 学习预览             | Redis 保存预览会话，并按学习项和文件版本协调 Office 转换                           | 版本化产物、锁内二次检查、30 秒租约自动续租                | Office 转换仍只允许 Development 环境显式启用                                   |
| 成员考核生成         | 按社团和规范化学期窗口加锁，在 Serializable 事务内生成或刷新草稿                    | 学期别名共用锁和既有记录；已公示记录不覆盖                  | 历史重复记录不在本任务清理                                                       |
...
多资源锁先去重，再按完整 Redis Key 的 Ordinal 顺序获取，部分失败时逆序条件释放。
长临界区使用自动续租；续租失败或 owner 不匹配会取消锁内业务，并在 Oracle 提交或
预览产物发布前再次确认租约。场地预约使用 15 秒租约、成员考核和 Office 转换使用
30 秒租约。缓存重建沿用短租约但不续租，锁竞争或 Redis 故障仍可回源 Oracle。
...
Redis:Features:PreviewSessions
...
| `VENUE_RESERVATIONS` 场地、状态、开始和结束时间索引     | 加速时间重叠查询                                   | 另开数据库 Issue 并经确认后评估；索引不能替代锁内复查     |
...
| 场地冲突       | 创建或审批时间重叠的预约                                                                    | 已有已通过预约时返回 409；并发审批由日期锁和事务内复查串行化 |
| 学习预览       | 调用 `POST /learning/items/{id}/preview-session` 保存 Cookie，再携带 Cookie 请求 `/preview` | 同版本并发仅转换一次；错用户、错学习项或过期会话返回未授权   |
