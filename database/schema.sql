-- 业务代码使用数据库生成主键。序列从较高区间起步，兼容 seeds 中保留的显式样例 ID。
...
CREATE SEQUENCE SEQ_BUDGET_ACCOUNTS START WITH 1000000 INCREMENT BY 1 NOCACHE NOCYCLE;
CREATE SEQUENCE SEQ_BUDGET_APPLICATIONS START WITH 1000000 INCREMENT BY 1 NOCACHE NOCYCLE;
CREATE SEQUENCE SEQ_BUDGET_REVIEW_RECORDS START WITH 1000000 INCREMENT BY 1 NOCACHE NOCYCLE;
CREATE SEQUENCE SEQ_BUDGET_TRANSACTIONS START WITH 1000000 INCREMENT BY 1 NOCACHE NOCYCLE;
...
CREATE SEQUENCE SEQ_IDEMPOTENCY_RECORDS START WITH 1000000 INCREMENT BY 1 NOCACHE NOCYCLE;
...
CREATE INDEX IX_BUDGET_APPLICATIONS_CLUB ON BUDGET_APPLICATIONS (club_id, application_status, submitted_at);
...
ALTER TABLE IDEMPOTENCY_RECORDS ADD CONSTRAINT UQ_IDEMPOTENCY_USER_SCOPE_KEY
  UNIQUE (user_id, operation_scope, request_key_hash);
...
ALTER TABLE IDEMPOTENCY_RECORDS ADD CONSTRAINT FK_IDEMPOTENCY_USER
...
ALTER TABLE BUDGET_ACCOUNTS ADD CONSTRAINT FK_BUDGET_ACCOUNTS_CLUB FOREIGN KEY (club_id) REFERENCES CLUBS (club_id) DEFERRABLE INITIALLY IMMEDIATE;
