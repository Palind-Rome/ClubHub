...
USING (
  SELECT 4 AS role_id, 'CLUB_OFFICER' AS role_code, '社团干部' AS role_name,
         'club' AS role_scope, '指定社团内角色，可管理招募、活动、通知、资源、项目任务和物资借还。' AS permission_desc
  FROM dual
) source
ON (target.role_id = source.role_id)
WHEN MATCHED THEN UPDATE SET
...
USING (
  SELECT 5 AS role_id, 'CLUB_LEADER' AS role_code, '社团负责人' AS role_name,
         'club' AS role_scope, '指定社团内最高业务角色，可维护社团信息、成员、社团内部角色和运营统计。' AS permission_desc
  FROM dual
) source
ON (target.role_id = source.role_id)
WHEN MATCHED THEN UPDATE SET
...
USING (
  SELECT 6 AS role_id, 'ADVISOR' AS role_code, '指导老师' AS role_name,
         'club' AS role_scope, '指定社团指导角色，可查看社团运营并审核活动、项目、经费和评价。' AS permission_desc
  FROM dual
) source
ON (target.role_id = source.role_id)
WHEN MATCHED THEN UPDATE SET
